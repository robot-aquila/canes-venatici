...
public class SpreadDetectorVega implements AXSpreadDetector {
    ...
    @Override
    public Optional<AXArbitrageSpread> detectSpread(AXAskSymbol askSymbol, AXBidSymbol bidSymbol) {
        AXMarketDepth buyers, sellers;
        var t = clock.instant();
        if (   ! Objects.isNull(buyers = registry.getMarketDepth(bidSymbol).orElse(null))
            && ! Objects.isNull(sellers = registry.getMarketDepth(askSymbol).orElse(null))
